...
pub fn all_routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/galleries",
            post(gallery::create_gallery).get(gallery::list_galleries),
        )
        .route(
            "/galleries/:id",
            get(gallery::get_gallery)
                .patch(gallery::update_gallery)
                .delete(gallery::delete_gallery),
        )
        .route("/galleries/:id/aliases", post(gallery::add_gallery_alias))
        .route(
            "/galleries/:id/aliases/:alias",
            delete(gallery::delete_gallery_alias),
        )
        // 全局图片管理
        .route("/images", get(image::list_images))
        .route(
            "/images/:id",
            get(image::get_image).patch(image::update_image),
        )
        .route(
            "/images/by-hash/:prefix",
            get(image::get_image_by_hash_prefix),
        )
        .route("/images/:id/aliases", post(image::add_image_alias))
        .route(
            "/images/:id/aliases/:alias",
            delete(image::delete_image_alias),
        )
        // 画廊-图片关联
        .route(
            "/galleries/:gallery_id/images",
            post(gallery_image::upload_image).get(gallery_image::list_gallery_images),
        )
        .route(
            "/galleries/:gallery_id/images/:image_id",
            delete(gallery_image::remove_image_from_gallery),
        )
        // 文件检索
